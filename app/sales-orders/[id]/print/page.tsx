...
import { getSalesOrderById, getSalesOrderItems, getCustomers } from '@/app/actions';
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (customersRes.data && orderData?.customer_id) {
          const found = (customersRes.data as any[]).find((c: any) => c.id === orderData.customer_id);
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body { background: #fff; font-family: 'Arial', sans-serif; font-size: 11pt; color: #111; }
        @page { size: A4; margin: 15mm 15mm 20mm 15mm; }
        @media print {
          html, body { width: 210mm; }
          .no-print { display: none !important; }
        }
        .page { width: 100%; max-width: 780px; margin: 0 auto; padding: 24px; }
        ...
        .doc-so-number { font-size: 11pt; color: #0ea5e9; font-weight: 600; margin-top: 4px; }
        .status-badge { display: inline-block; padding: 2px 10px; border-radius: 4px; font-size: 8pt; font-weight: 700; letter-spacing: 1px; margin-top: 6px; background: ${statusColor.bg}; color: ${statusColor.text}; border: 1px solid ${statusColor.border}; }
      ...
      <div className="page">
        ...
        <div className="doc-header">
          ...
          <div className="doc-title-block">
            ...
            <div className="doc-so-number">{order.so_number}</div>
        ...
        <div className="meta-grid">
          <div className="meta-box">
            ...
            {order.actual_delivery_date && (
              <div className="meta-row"><span className="label">Delivered</span><span className="value">{fmtDate(order.actual_delivery_date)}</span></div>
            )}
            {order.payment_terms && (
          ...
          <div className="meta-box">
            <div className="meta-box-title">Order Summary</div>
            <div className="meta-row"><span className="label">Total Items</span><span className="value">{items.length}</span></div>
            <div className="meta-row"><span className="label">Total Qty</span><span className="value">{fmtQty(items.reduce((s, i) => s + Number(i.quantity_ordered), 0))}</span></div>
            <div className="meta-row"><span className="label">Grand Total</span><span className="value">{fmt(Number(order.total_amount))}</span></div>
          </div>
        </div>
        ...
        <div className="addr-grid">
          <div className="addr-box">
            <div className="addr-box-title">From (Seller)</div>
          ...
          <div className="addr-box">
            <div className="addr-box-title">Bill To (Customer)</div>
            <div className="addr-name">{customer?.name || '—'}</div>
            {customer?.contact_person && <div className="addr-detail">Attn: {customer.contact_person}</div>}
            {customer?.email && <div className="addr-detail">{customer.email}</div>}
            {customer?.phone && <div className="addr-detail">{customer.phone}</div>}
            {customer?.address && <div className="addr-detail">{customer.address}</div>}
          </div>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style={{ width: '13%' }}>Qty Shipped</th>
            ...
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id || i}>
                  ...
                  <td>{fmtQty(Number(item.quantity_shipped || 0))}</td>
        ...
        <div className="sig-section">
          <div className="sig-block">
            ...
            <div className="sig-role">Sales Officer</div>
          </div>
          <div className="sig-block">
            <div style={{ height: 32 }} />
          ...
          <div className="sig-block">
            ...
            <div className="sig-role">Customer Representative</div>
          </div>
        </div>

        {/* ── Footer ── */}
        <div className="doc-footer">
          This is an official Sales Order issued by {company?.name || 'the company'}.
          &nbsp;&bull;&nbsp; Generated {new Date().toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' })}
          &nbsp;&bull;&nbsp; {order.so_number}
        </div>
      </div>
    </>
  );
